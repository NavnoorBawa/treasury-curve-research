...
| AUD-016 | Medium | Weekly research workflow | The Weekly view was fixed to the latest official week, preventing direct review of prior weekly records and their contemporaneous curve classifications. | Added previous/next navigation, a Monday date selector, latest-week reset, and canonical `week` URL state. Historical selections recalculate weekly, YTD, and year-end results from an observation-truncated dataset; deterministic tests verify normalization, future clamping, actual-only rows, and no-look-ahead forecast origins. | RESOLVED |
| AUD-017 | Low | Local observability | The production-mode localhost preview mounted Vercel Analytics, whose deployment-only script returned 404 locally and created a Lighthouse console error. | Analytics now mounts only on HTTPS non-local hosts. The historical-week production-mode Lighthouse rerun scores 92 performance and 100 accessibility, best practices, and SEO with no failed binary audits. | RESOLVED |
...
- Historical-week workflow: previous/next, direct-date, latest reset, canonical URL restoration, 30Y-publication-gap behavior, and 390 px layout passed without alerts or document overflow.
- Historical-week Lighthouse: performance 92, accessibility 100, best practices 100, and SEO 100 in the local production build; no failed binary audits.
